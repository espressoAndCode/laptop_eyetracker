...
def init_cv():
    ...
    face_detector = cv2.CascadeClassifier(
        os.path.join("filters", "haarcascade_frontalface_default.xml"))
    eye_detector = cv2.CascadeClassifier(os.path.join("filters", 'haarcascade_eye.xml'))
...
def get_keypoints(detector, frame, frame_gray, threshold, previous_area, previous_keypoint):

    keypoints = process_eye(frame_gray, threshold, detector,
                                    prevArea=previous_area)
    if keypoints and type(keypoints) is list:
        print(f'Keypoints type: {keypoints}')
